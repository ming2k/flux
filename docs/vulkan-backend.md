...
This document covers the Vulkan objects vgfx owns, the choices made in structuring them, and the invariants that keep the codebase correct.
...
vgfx uses a custom dynamic memory manager designed for high-performance 2D rendering.

### Linear Buffer Pool (`src/vk/memory.c`)
Instead of fixed vertex limits, vgfx implements a per-frame linear allocator (`vg_vbuf_pool`) for `HOST_VISIBLE | HOST_COHERENT` memory.
...
### Image Memory
User images and the Glyph Atlas are allocated in `DEVICE_LOCAL` memory for maximum sampling performance. vgfx uses staging buffers and one-time-submit command buffers to upload CPU pixel data to the GPU.

## Draw Call Batching

The renderer automatically batches primitives to minimize Vulkan overhead.
- **Grouping:** Sequential drawing operations (`vg_fill_path`, `vg_stroke_path`) are grouped into a single `vkCmdDraw` if they share the same `vg_paint` properties (color, etc.).
...
vgfx uses specialized pipelines for different primitive types:
...
- **Updates:** If a glyph is missing during recording, vgfx immediately renders it via FreeType and issues a `vkCmdCopyBufferToImage` to update the atlas before the frame is submitted.

## Descriptor Management

vgfx uses a **Per-Frame Descriptor Pool** strategy:
...
vgfx follows a standard two-frame in-flight architecture:
...
`vg_device_init` scores devices based on type (Discrete > Integrated > Virtual). It requires:
