...
vgfx is a **low-level 2D graphics substrate** for a Wayland-first UI
...
-----------------------------------------------
vgfx: images, paths, glyph runs, frame recording
-----------------------------------------------
...
- **Skia**: reference point for breadth and GPU-first 2D rendering.
  vgfx follows the idea of a unified image/vector/text backend, but it
  stays narrower and more explicit.
- **Cairo**: reference point for a stable drawing vocabulary. vgfx
...
- **Pango / HarfBuzz**: these belong above vgfx. They shape text and
  choose fonts; vgfx consumes the resulting glyph stream and renders it.
...
the caller. vgfx deliberately stops below that line.
